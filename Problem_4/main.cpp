...
// Add Uttara bus lines to the graph
void addUttaraBusToGraph(vector<tuple<vector<Point>, string, string>>& busLines, 
                         map<int, vector<pair<int, double>>>& adj) {
    int edgeCount = 0;
    ...
    cout << "Added " << edgeCount << " Uttara bus edges to graph" << endl;
}

// Add Bikolpo bus lines to the graph
void addBikolpoBusToGraph(vector<tuple<vector<Point>, string, string>>& busLines, 
                          map<int, vector<pair<int, double>>>& adj) {
    int edgeCount = 0;
    ...
    cout << "Added " << edgeCount << " Bikolpo bus edges to graph" << endl;
}
...
// Generate text directions with time tracking (for Problem 4)
// Parameters:
//   path - vector of node IDs
//   srcLat, srcLon - source coordinates
//   destLat, destLon - destination coordinates
//   srcSnapped - whether source is exactly on graph
//   destSnapped - whether destination is exactly on graph
//   totalCost - total cost in Taka
//   totalDist - total distance in km
//   startTime - journey start time
//   filename - output text file name
void generateTextDirectionsWithTime(vector<int>& path, double srcLat, double srcLon, 
    ...
    // Generate turn-by-turn directions with time and cost tracking
    for(int i = 0; i < path.size() - 1; i++) {
        int u = path[i];
        int v = path[i + 1];
        Point from = nodes[u];
        ...
        if(i == 0 && !srcSnapped) {
            ...
            int walkTime = (int)((walkDist / 2.0) * 60);  // 2 km/h walking speed
            currentTime = addMinutes(currentTime, walkTime);
            file << "Time: " << currentTime.hour << ":" << setfill('0') << setw(2) << currentTime.minute 
                 << ". Cost: 0 Taka. Walk from Source (" << srcLat << ", " << srcLon 
                 << ") to (" << from.lat << ", " << from.lon << ")" << endl;
        }
        
        // Handle public transport waiting time and check operating hours
        if(info.type != "car") {
        ...
        file << "Time: " << currentTime.hour << ":" << setfill('0') << setw(2) << currentTime.minute 
             << ". Cost: " << setprecision(2) << info.cost << " Taka. ";
        ...
        // Add travel time for this segment (30 km/h average speed)
        int travelTime = (int)((info.distance / 30.0) * 60);
        ...
        if(i == path.size() - 2 && !destSnapped) {
            ...
            currentTime = addMinutes(currentTime, walkTime);
            file << "Time: " << currentTime.hour << ":" << setfill('0') << setw(2) << currentTime.minute 
                 << ". Cost: 0 Taka. Walk from (" << to.lat << ", " << to.lon 
                 << ") to Destination (" << destLat << ", " << destLon << ")" << endl;
        }
    }
...
int main() {
    ...
    cout << "=== Building Graph for Problem 4 ===" << endl;
    ...
    generateTextDirectionsWithTime(path, srcLat, srcLon, destLat, destLon, 
                          srcSnapped, destSnapped, cost[destNode], totalDist, 
                          startTime, "route_output.txt");
    generateKML(path, "route.kml");
    
    cout << "\nDone! You can now:" << endl;
    cout << "  - View route_output.txt for time-aware turn-by-turn directions" << endl;
