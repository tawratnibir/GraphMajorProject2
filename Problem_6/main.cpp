...
// Speed parameters for Problem 6 (km/h)
const double CAR_SPEED = 20.0;
const double METRO_SPEED = 15.0;
const double UTTARA_SPEED = 12.0;
const double BIKOLPO_SPEED = 10.0;
...
// ============== GRAPH BUILDING (Problem 6) ==============
// Build graph with realistic speeds for each transport type
void buildGraphSpeed(vector<vector<Point>>& roads, map<int, vector<pair<int, double>>>& adj) {
    int edgeCount = 0;
    ...
    for(auto& road : roads) {
        for(size_t i = 0; i < road.size() - 1; i++) {
            ...
            double timeWeight = distance / CAR_SPEED;  // hours
    ...
    cout << "Built speed-differentiated road graph with " << nodes.size() << " nodes and " 
         << edgeCount << " edges" << endl;
}

void addMetroToGraphSpeed(vector<tuple<vector<Point>, string, string>>& metroLines, 
                          map<int, vector<pair<int, double>>>& adj) {
    int edgeCount = 0;
    ...
    for(auto& metroLine : metroLines) {
        ...
        double timeWeight = distance / METRO_SPEED;  // hours
    ...
    cout << "Added " << edgeCount << " metro edges (speed-differentiated)" << endl;
}

void addUttaraBusToGraphSpeed(vector<tuple<vector<Point>, string, string>>& busLines, 
                              map<int, vector<pair<int, double>>>& adj) {
    int edgeCount = 0;
    ...
    for(auto& busLine : busLines) {
        ...
        double timeWeight = distance / UTTARA_SPEED;  // hours
    ...
    cout << "Added " << edgeCount << " Uttara bus edges (speed-differentiated)" << endl;
}

void addBikolpoBusToGraphSpeed(vector<tuple<vector<Point>, string, string>>& busLines, 
                               map<int, vector<pair<int, double>>>& adj) {
    int edgeCount = 0;
    ...
    for(auto& busLine : busLines) {
        ...
        double timeWeight = distance / BIKOLPO_SPEED;  // hours
    ...
    cout << "Added " << edgeCount << " Bikolpo bus edges (speed-differentiated)" << endl;
}
...
// ============== TEXT DIRECTIONS OUTPUT (Problem 6) ==============

// Generate text directions for Problem 6 (realistic speeds per transport type)
void generateTextDirectionsRealisticSpeed(vector<int>& path, double srcLat, double srcLon, 
    ...
    file << "Note: Uses realistic speeds - Car:" << CAR_SPEED << " km/h, Metro:" << METRO_SPEED 
         << " km/h, Uttara:" << UTTARA_SPEED << " km/h, Bikolpo:" << BIKOLPO_SPEED << " km/h" << endl;
    file << endl;
    ...
    for(int i = 0; i < path.size() - 1; i++) {
        ...
        // Calculate travel time based on transport-specific speed
        double segmentTimeMinutes;
        if(info.type == "car") {
            segmentTimeMinutes = (info.distance / CAR_SPEED) * 60.0;
        } else if(info.type == "metro") {
            segmentTimeMinutes = (info.distance / METRO_SPEED) * 60.0;
        } else if(info.type == "uttara_bus") {
            segmentTimeMinutes = (info.distance / UTTARA_SPEED) * 60.0;
        } else {  // bikolpo_bus
            segmentTimeMinutes = (info.distance / BIKOLPO_SPEED) * 60.0;
        }
...
int main() {
    ...
    cout << "=== Building Graph for Problem 6 ===" << endl;
    ...
    generateTextDirectionsRealisticSpeed(path, srcLat, srcLon, destLat, destLon, 
                          srcSnapped, destSnapped, cost[destNode], totalDist, 
                          startTime, "route_output.txt");
    generateKML(path, "route.kml");
    
    cout << "\nDone! You can now:" << endl;
    cout << "  - View route_output.txt for time-minimized directions (realistic speeds)" << endl;
